...
#-----------------------------------------------------------------------------------------------------------#
#-----------------------------------------------------------------------------------------------------------#
safe_solve <- function(mat, lambda = 1e-6) {
      solve(mat + lambda * diag(nrow(mat)))
  }
                 ...
                 type="linear",tau=0.5,alpha=NULL,gamma=10,sd=FALSE,Kfolds=5)
{
  ...
  weights_loss <- as.vector(c1+(1-c1)*t(unlabelled_Z_mean)%*%safe_solve(labelled_Z_seondmoment)%*%t(labelled_Z))

  
  
  # estimate of the target parameter
  if (type=="linear"){
    hattheta_PSSE <- as.vector(safe_solve(t(cbind(rep(1,n),labelled_data[,-1]))%*%
  ...
  if(sd==TRUE){
    ...
    if(type=="linear"){
      ...
      X_secondmoment_inverse=safe_solve(t(cbind(rep(1,N),unlabelled_data))%*%cbind(rep(1,N),unlabelled_data)/N) 
      ...
      for(k in 1:Kfolds){
        ...
        L_firstder_projection_cof <- safe_solve(t(Zt_train)%*%Zt_train/nrow_Xt_train)%*%
      ...
      L_firstder_projection_cof_total <- safe_solve(t(labelled_Z)%*%labelled_Z/n)%*%t(labelled_Z)%*%L_firstder_total/n
    ...
    if(type=="logistic"){
      ...
      X_secondmoment_inverse=safe_solve(t(weights_second_derivative*cbind(rep(1,N),unlabelled_data))%*%cbind(rep(1,N),unlabelled_data)/N) 
    ...
    if(type=="quantile"){
      ...
      for(k in 1:Kfolds){
        ...
        L_firstder_projection_cof <- safe_solve(t(Zt_train)%*%Zt_train)%*%
      ...
      L_firstder_projection_cof_total <- safe_solve(t(labelled_Z)%*%labelled_Z)%*%t(labelled_Z)%*%L_firstder_total
      W2_total <- (1-c1)*unlabelled_Z%*%L_firstder_projection_cof_total
      ...
      if(class(try(safe_solve(hat_M_semi),silent=T))[1]=="try-error"){
        X_secondmoment_inverse <- safe_solve((hat_M_semi+t(hat_M_semi))/2)
      } else{
        X_secondmoment_inverse<- safe_solve(hat_M_semi)
      }
...
GBIC_ppo <- function (Y_new,covariates_matrix,order_poly){
  ...
  if(class(try(safe_solve(design_matrix),silent=T))[1]=="try-error"){
    ...
    for (j in 1:d){
      ...
      cova_constrast <- 1/sigmahat_square[j]*safe_solve(design_matrix)%*%Bhat_j
...
EASE<-function(labelled_data,unlabelled_data,K,H,r)
{
  ...
  for (k in 1:K)
  {
    ...
    Sigma_sqrt <- (eigen_X_train_covariance$vectors)%*%(lamda_sqrt*safe_solve(eigen_X_train_covariance$vectors))
    Sigma_negative_sqrt <- safe_solve(Sigma_sqrt)  # calculate the Sigma^{-1/2}
...
DRESS<- function(labelled_data,unlabelled_data,type="linear",tau=0.5,L,sd=FALSE,Kfolds=5)
{
  ...
  while(alpha_distance >tol& i<=max_i){
    ...
    alpha_new <- as.vector(alpha_initial-safe_solve(alpha_second_derivative)%*%alpha_first_derivative)
